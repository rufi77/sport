...
ski_season <- (all %>% filter(date >= "2012-12-31") %>% arrange(season) %>% group_by(season) %>% 
                 summarise(Hm = sum(hm_diff, na.rm = TRUE), n = sum(is.na(unique(date)) == FALSE))) %>% 
                 add_row(season = c("Winter 13/14", "Winter 14/15"), Hm = 0, n = 0) %>% arrange(season)
```
...
- Total Hoehenmeter: `r tail(ski_season, 1)$Hm`.
...
all_m <- (all_m0 %>% arrange(date) %>% group_by(date) %>% slice(1) %>% select(-season))
cuts <- parse_date_time(x = "2018-06-30", orders = "ymd") + years(x = seq.int(from = 0, to = s - 18, by = 1))
labs <- c(paste("Winter ", 18:(s - 1), "/", 19:s, sep = ""))

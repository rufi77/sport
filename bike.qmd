...
t <- (touren %>% filter(sport == selectsport & year(touren$date) == cy & grepl("Alexander", beg) == FALSE) %>% group_by(date) %>% slice_tail() %>% hm_print())
```
...
```{r, echo = FALSE, results = 'asis', message = FALSE, eval = TRUE}
thm <- (touren %>% filter(sport == selectsport & year(touren$date) == cy) %>% group_by(date) %>% arrange(desc(time), .by_group = TRUE) %>% hm_print())
...
```{r, echo = FALSE, results = 'asis', message = FALSE, eval = TRUE}
t1 <- (touren %>% filter(sport == selectsport & year(touren$date) == cy & grepl("Alexander", beg) == FALSE) %>% group_by(date) %>% hm_print())
t1 <- (t1 %>% group_by(paste(start, end, sep = "")) %>% filter(is.na(hm_h) == FALSE) %>% arrange(desc(hm_h)) %>% ungroup())
t1 <- (t1 %>% hm_print())
...
```{r, echo = FALSE, results = 'asis', message = FALSE, eval = TRUE}
t <- (touren %>% filter(sport == selectsport & year(touren$date) == cy) %>% arrange(date))

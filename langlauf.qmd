...
# Kilometer pro Saison

```{r, echo = FALSE, fig.cap = "", fig.align = "center", fig.width = 5, fig.height = 3}
ggplot(data = ski_season, aes(x = season, y = Km, group = 1)) +
geom_line(color = "red") +
geom_point() +
theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
labs(x = "Saison", y = "Kilometer")
```
...
```{r, echo = FALSE, results = 'asis', message = FALSE, eval = TRUE}
t1 <- (t %>% arrange(date, desc(time)) %>% group_by(date) %>% slice(1) %>% rowid_to_column("Nr") %>% mutate(km = km_manual) %>% mutate(km_h = round(km_h, 1)) %>% select(start, end, date, time, hm_diff, km, km_h, pace))
pander(t1)
```
...
```{r, echo = FALSE, results = 'asis', message = FALSE, eval = TRUE}
t1 <- (t %>% arrange(desc(pace)) %>% group_by(date) %>% slice(1) %>% rowid_to_column("Nr") %>% mutate(km = km_manual) %>% mutate(km_h = round(km_h, 1)) %>% select(start, end, date, time, hm_diff, km, km_h, pace))
pander(t1)
```
